  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-950 via-indigo-900 to-purple-900">
      ...
      <div className="relative py-20 bg-gradient-to-r from-purple-900/50 to-indigo-900/50">
        ...
        <div className="absolute bottom-10 right-10 text-3xl text-purple-300 animate-bounce">🕯️</div>
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-6xl mx-auto">
          {/* Controls */}
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-purple-300/20 mb-8">
              ...
              {viewMode === 'month' && (
                <div className="flex space-x-2">
                  <select
                    ...
                    className="px-3 py-2 bg-white/10 border border-purple-300/30 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-golden"
                  >
                    {monthNames.map((month, index) => (
                      <option key={index} value={index + 1} className="bg-purple-900">
                        {month}
                      </option>
                    ))}
                  </select>
                  <input
                    ...
                    min="2020"
                    max="2030"
                    className="px-3 py-2 bg-white/10 border border-purple-300/30 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-golden w-24"
                  />
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {events.map((event, index) => (
              <div 
                key={index}
                className="bg-white/10 backdrop-blur-md rounded-2xl border border-purple-300/20 overflow-hidden hover:transform hover:scale-105 transition-all duration-300"
              >
                {/* Event Header */}
                <div className={`p-6 ${event.isSpecial ? 'bg-gradient-to-r from-golden/20 to-yellow-300/20' : 'bg-purple-800/30'}`}>
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {event.tags.map((tag, idx) => (
                      <span 
                        key={idx}
                        className="px-3 py-1 bg-purple-600/30 text-purple-200 text-xs rounded-full border border-purple-400/30"
                      >
                        {tag}
                      </span>
                    ))}
          ...
          {/* No events message */}
          {events.length === 0 && (
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-12 border border-purple-300/20 text-center">
          ...
          {/* Legend */}
          <div className="mt-8 bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-purple-300/20">
            <h3 className="text-xl font-bold text-white mb-4">📚 Chú giải</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="space-y-2">
                <div className="flex items-center">
                  <span className="mr-2">⭐</span>
                  <span className="text-purple-200">Ngày lễ đặc biệt quan trọng</span>
                </div>
                <div className="flex items-center">
                  <span className="mr-2">🙏</span>
                  <span className="text-purple-200">Ngày cúng bái, tâm linh</span>
                </div>
                <div className="flex items-center">
                  <span className="mr-2">🌸</span>
                  <span className="text-purple-200">Lễ hội văn hóa truyền thống</span>
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex items-center">
                  <span className="mr-2">🌕</span>
                  <span className="text-purple-200">Ngày rằm, trăng tròn</span>
                </div>
                <div className="flex items-center">
                  <span className="mr-2">📅</span>
                  <span className="text-purple-200">Sự kiện theo âm lịch</span>
                </div>
                <div className="flex items-center">
                  <span className="mr-2">🎊</span>
                  <span className="text-purple-200">Lễ hội lớn của dân tộc</span>
                </div>
              </div>
            </div>
          </div>
