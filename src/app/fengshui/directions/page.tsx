...
const directionsData: Record<string, DirectionInfo> = {
  'North': {
    ...
    benefits: ['Phát triển sự nghiệp', 'Tăng cường trí tuệ', 'Cải thiện danh tiếng'],
  ...
  'Northeast': {
    ...
    benefits: ['Tăng cường học vấn', 'Phát triển tâm linh', 'Cải thiện sức khỏe'],
  ...
  'East': {
    ...
    benefits: ['Hòa hợp gia đình', 'Tăng cường sức khỏe', 'Phát triển bản thân'],
  ...
  'Southeast': {
    ...
    benefits: ['Gia tăng tài lộc', 'Phát triển kinh doanh', 'Mở rộng mối quan hệ'],
  ...
  'South': {
    ...
    benefits: ['Nâng cao danh tiếng', 'Đạt được thành công', 'Tăng cường năng lượng'],
  ...
  'Southwest': {
    ...
    benefits: ['Cải thiện tình yêu', 'Tăng cường mối quan hệ', 'Ổn định hôn nhân'],
  ...
  'Northwest': {
    ...
    benefits: ['Gặp được quý nhân', 'Tăng cường quyền lực', 'Cải thiện lãnh đạo'],
...
const calculateKuaNumber = (birthYear: number, gender: 'male' | 'female'): number => {
...
const getDirectionsForKua = (kuaNumber: number): PersonalDirections['directions'] => {
...
export default function FengshuiDirectionsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'detailed' | 'recommendations' | 'compass'>('overview');
  ...
  const analyzeDirections = () => {
    ...
    setTimeout(() => {
      ...
      const recommendations = {
        ...
        working: [...directions.excellent, ...directions.good].map(d => d.name),
  ...
  const getDirectionColor = (level: string): string => {
    switch (level) {
      ...
      case 'bad': return 'border-red-500 bg-red-500/10';
  ...
  const getLevelText = (level: string): string => {
    switch (level) {
      ...
      case 'neutral': return 'Trung Bình';
      case 'bad': return 'Nên Tránh';
      default: return '';
    }
  };
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-900/20 to-cyan-900/20 py-20">
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-xl md:text-2xl text-gray-300 max-w-3xl mx-auto mb-4 leading-relaxed">
            <strong className="text-golden">Xác định hướng tốt xấu</strong> theo tuổi và mệnh phong thủy
          </p>
          
          <p className="text-lg text-gray-400 max-w-2xl mx-auto leading-relaxed">
            Tìm hướng may mắn cho giường ngủ, bàn làm việc và cửa chính dựa trên số Kua cá nhân
          </p>
        </div>
      </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-sm rounded-3xl p-8 border border-gray-700/20">
              <div className="text-center mb-8">
                ...
                <div className="text-golden font-semibold mb-4">
                  Nhóm {analysis.eastWestGroup === 'East' ? 'Đông' : 'Tây'} • Năm sinh: {analysis.birthYear} • {analysis.gender === 'male' ? 'Nam' : 'Nữ'}
                </div>
                <p className="text-gray-300 leading-relaxed max-w-3xl mx-auto">
                  Số Kua của bạn xác định nhóm hướng phù hợp. Nhóm {analysis.eastWestGroup === 'East' ? 'Đông' : 'Tây'} có những hướng may mắn riêng biệt.
                </p>
              </div>
            </div>
            ...
            {activeTab === 'overview' && (
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                {Object.entries(analysis.directions).map(([level, directions]) => (
                  <div key={level} className={`rounded-3xl p-6 border ${getDirectionColor(level)}`}>
                      ...
                      {directions.map((direction, index) => (
                        <div key={index} className="bg-gray-700/30 rounded-lg p-3">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-lg">{direction.symbol}</span>
                            <span className="font-semibold text-white">{direction.name}</span>
                          </div>
                          <div className="text-gray-300 text-sm">{direction.energy}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                      ...
                      {directions.map((direction, index) => (
                        <div key={index} className="bg-gray-700/30 rounded-lg p-4">
                          <div className="flex items-center gap-3 mb-3">
                            <span className="text-2xl">{direction.symbol}</span>
                            <div>
                              <h4 className="font-bold text-white">{direction.name}</h4>
                              <div className="text-sm text-gray-400">{direction.element} • {direction.color}</div>
                            </div>
                          </div>
                          <div className="mb-3">
                            <div className="text-golden font-semibold mb-1">Năng lượng:</div>
                            <div className="text-gray-300 text-sm">{direction.energy}</div>
                          </div>
                          <div className="mb-3">
                            <div className="text-golden font-semibold mb-1">Lợi ích:</div>
                          ...
                          <div>
                            <div className="text-golden font-semibold mb-1">Hoạt động phù hợp:</div>
            ...
            {activeTab === 'recommendations' && (
              <div className="grid md:grid-cols-2 gap-6">
                {Object.entries(analysis.recommendations).map(([category, directions]) => (
                  <div key={category} className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-sm rounded-3xl p-6 border border-gray-700/20">
                    ...
                    <div className="space-y-2">
                      {directions.map((direction, index) => (
                        <div key={index} className={`p-3 rounded-lg ${
                          category === 'avoid' ? 'bg-red-500/10 border border-red-500/20' : 'bg-green-500/10 border border-green-500/20'
                        }`}>
                          <span className={`font-semibold ${
                            category === 'avoid' ? 'text-red-400' : 'text-green-400'
                          }`}>
                            {direction}
                          </span>
                        </div>
                    ...
                    <div className="mt-4 text-gray-400 text-sm">
                      {category === 'sleeping' && 'Đầu giường hướng về các hướng này'}
                      {category === 'working' && 'Ngồi quay mặt về các hướng này khi làm việc'}
                      {category === 'entrance' && 'Cửa chính nên hướng về các hướng này'}
                      {category === 'avoid' && 'Tránh các hướng này cho các hoạt động quan trọng'}
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="max-w-2xl mx-auto">
                  <div className="relative w-full aspect-square bg-gradient-to-br from-gray-700 to-gray-800 rounded-full p-8">
                    ...
                    <div className="absolute inset-8 rounded-full border-4 border-golden/30">
                      {Object.entries(directionsData).map(([key, direction], index) => {
                        const angle = index * 45;
                        const isGood = [...analysis.directions.excellent, ...analysis.directions.good].some(d => d.name === direction.name);
                        const isBad = analysis.directions.bad.some(d => d.name === direction.name);
                    ...
                    <div className="absolute inset-1/2 transform -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-golden rounded-full flex items-center justify-center">
                      <span className="text-black font-bold text-lg">{analysis.kuaNumber}</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-8 grid grid-cols-3 gap-4 text-center">
                    ...
                    <div className="bg-yellow-500/10 rounded-lg p-3 border border-yellow-500/20">
                      ...
                      <div className="text-yellow-400 font-semibold">Trung Bình</div>
                    </div>
                    <div className="bg-red-500/10 rounded-lg p-3 border border-red-500/20">
                      <div className="w-6 h-6 bg-red-500 rounded-full mx-auto mb-2"></div>
                      <div className="text-red-400 font-semibold">Nên Tránh</div>
                    </div>
                  </div>
                </div>
              </div>
