...
const elements: Record<string, ElementInfo> = {
  'Kim': {
    ...
    personality: 'Người mệnh Kim thường có tính cách mạnh mẽ, quyết đoán và có khả năng lãnh đạo tốt',
  ...
  'Mộc': {
    ...
    characteristics: 'Linh hoạt, sáng tạo, có khả năng phát triển, yêu thiên nhiên',
    personality: 'Người mệnh Mộc thường nhân hậu, có lòng từ bi và khả năng thích nghi cao',
  ...
  'Thủy': {
    ...
    personality: 'Người mệnh Thủy thường thông minh, linh hoạt và có khả năng thích nghi tốt',
    career: ['Truyền thông', 'Du lịch', 'Thương mại', 'Nghiên cứu', 'Công nghệ'],
  ...
  'Hỏa': {
    ...
    characteristics: 'Nhiệt huyết, năng động, có khả năng lãnh đạo, thích thể hiện',
    personality: 'Người mệnh Hỏa thường năng động, nhiệt tình và có khả năng truyền cảm hứng',
  ...
  'Thổ': {
    ...
    personality: 'Người mệnh Thổ thường trung thực, đáng tin cậy và có khả năng tổ chức tốt',
...
const colorMeaningsData: Record<string, { meaning: string; usage: string }> = {
  'Đỏ': { meaning: 'Năng lượng, may mắn, thành công', usage: 'Phòng khách, phòng làm việc, trang phục quan trọng' },
  'Xanh lá': { meaning: 'Phát triển, sức khỏe, tươi mới', usage: 'Phòng ngủ, khu vườn, không gian thư giãn' },
  'Vàng': { meaning: 'Giàu có, quyền lực, trí tuệ', usage: 'Phòng làm việc, khu vực tài chính, trang sức' },
  'Đen': { meaning: 'Bí ẩn, quyền lực, thanh lịch', usage: 'Trang phục công sở, phụ kiện, không gian nghiêm túc' },
  'Trắng': { meaning: 'Tinh khiết, bình an, khởi đầu mới', usage: 'Phòng ngủ, phòng tắm, trang phục cưới' },
  'Xanh dương': { meaning: 'Bình yên, trí tuệ, tin cậy', usage: 'Phòng học, phòng làm việc, không gian thư giãn' },
  'Tím': { meaning: 'Quý phái, tâm linh, sáng tạo', usage: 'Phòng thiền, không gian nghệ thuật, phụ kiện' },
  'Hồng': { meaning: 'Tình yêu, dịu dàng, hạnh phúc', usage: 'Phòng ngủ, không gian lãng mạn, trang phục dạ tiệc' }
};
...
export default function FengshuiColorsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'recommendations' | 'meanings' | 'calendar'>('overview');
  ...
  const analyzeColors = () => {
    ...
    setTimeout(() => {
      ...
      const colorRecommendations = {
        clothing: [...elementInfo.colors.primary, ...elementInfo.colors.secondary.slice(0, 2)],
        home: elementInfo.colors.primary,
        office: [elementInfo.colors.primary[0], elementInfo.colors.secondary[0]],
      ...
      const colorMeanings = elementInfo.colors.primary.map(color => ({
        ...
        usage: colorMeaningsData[color]?.usage || 'Sử dụng trong cuộc sống hàng ngày'
      }));

      const monthlyColors = [
        { month: 'Tháng 1-3', color: elementInfo.colors.primary[0], reason: 'Khởi đầu năm mới với năng lượng tích cực' },
        { month: 'Tháng 4-6', color: elementInfo.colors.primary[1] || elementInfo.colors.primary[0], reason: 'Phát triển và tăng trưởng' },
        { month: 'Tháng 7-9', color: elementInfo.colors.secondary[0], reason: 'Cân bằng và ổn định giữa năm' },
        { month: 'Tháng 10-12', color: elementInfo.colors.primary[0], reason: 'Kết thúc năm với may mắn' }
      ];
  ...
  return (
    <div className="min-h-screen">
      {/* Header */}
      <div className="relative overflow-hidden bg-gradient-to-r from-green-900/20 to-emerald-900/20 py-20">
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-xl md:text-2xl text-gray-300 max-w-3xl mx-auto mb-4 leading-relaxed">
            <strong className="text-golden">Tìm màu sắc phù hợp</strong> theo ngũ hành và năm sinh
          </p>
          
          <p className="text-lg text-gray-400 max-w-2xl mx-auto leading-relaxed">
            Khám phá màu sắc may mắn cho trang phục, nhà ở và cuộc sống dựa trên mệnh ngũ hành
          </p>
        </div>
      </div>
        ...
        {analysis && (
          <div className="space-y-8">
              ...
              <div className="text-center mb-8">
                ...
                <div className="grid md:grid-cols-3 gap-4 text-sm">
                  <div className="bg-green-500/10 rounded-lg p-3 border border-green-500/20">
                    <div className="text-green-400 font-semibold mb-1">Hướng may mắn</div>
                    <div className="text-gray-300">{analysis.elementInfo.direction}</div>
                  </div>
                  <div className="bg-blue-500/10 rounded-lg p-3 border border-blue-500/20">
                    <div className="text-blue-400 font-semibold mb-1">Mùa thuận lợi</div>
                    <div className="text-gray-300">{analysis.elementInfo.season}</div>
                  </div>
                  <div className="bg-purple-500/10 rounded-lg p-3 border border-purple-500/20">
                    <div className="text-purple-400 font-semibold mb-1">Số may mắn</div>
                    <div className="text-gray-300">{analysis.elementInfo.luckyNumbers.join(', ')}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            {activeTab === 'overview' && (
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="bg-gradient-to-br from-blue-900/20 to-gray-900/50 backdrop-blur-sm rounded-3xl p-6 border border-blue-700/20">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {analysis.elementInfo.colors.secondary.map((color, index) => (
                      <div key={index} className="flex items-center gap-3 p-3 bg-gray-700/30 rounded-lg">
                        <div className={`w-8 h-8 rounded-full ${getColorStyle(color)}`}></div>
                        <span className="text-gray-300 font-medium">{color}</span>
                      </div>
                ...
                <div className="bg-gradient-to-br from-golden/10 to-gray-900/50 backdrop-blur-sm rounded-3xl p-6 border border-golden/20">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      <span className="text-golden font-semibold">Tính cách: </span>
                      <span className="text-gray-300">{analysis.elementInfo.personality}</span>
                    </div>
                    <div>
                      <span className="text-golden font-semibold">Sức khỏe: </span>
                      <span className="text-gray-300">{analysis.elementInfo.health}</span>
                    </div>
                    <div>
                      <span className="text-golden font-semibold">Mối quan hệ: </span>
                      <span className="text-gray-300">{analysis.elementInfo.relationships}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'recommendations' && (
              <div className="grid md:grid-cols-2 gap-6">
                {Object.entries(analysis.colorRecommendations).map(([category, colors]) => (
                ...
                {analysis.colorMeanings.map((item, index) => (
                  <div key={index} className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-sm rounded-3xl p-6 border border-gray-700/20">
                    <div className="flex items-start gap-4">
                      <div className={`w-16 h-16 rounded-full ${getColorStyle(item.color)} flex-shrink-0`}></div>
                      <div className="flex-1">
                        <h3 className="text-xl font-bold text-white mb-2">{item.color}</h3>
                        <p className="text-gray-300 mb-3">{item.meaning}</p>
                        <div className="bg-golden/10 rounded-lg p-3 border border-golden/20">
                          <div className="text-golden font-semibold mb-1">Cách sử dụng:</div>
                          <div className="text-gray-300 text-sm">{item.usage}</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {analysis.monthlyColors.map((item, index) => (
                  <div key={index} className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-sm rounded-3xl p-6 border border-gray-700/20">
                    <div className="flex items-center gap-4 mb-4">
                      <div className={`w-12 h-12 rounded-full ${getColorStyle(item.color)}`}></div>
                      <div>
                        <h3 className="text-lg font-bold text-white">{item.month}</h3>
                        <div className="text-golden font-semibold">{item.color}</div>
                      </div>
                    </div>
