...
const monthlyThemes: Record<number, any> = {
  1: { energy: "Khởi đầu mới", focus: "Lãnh đạo và sáng tạo", color: "from-red-500 to-pink-500" },
  2: { energy: "Hợp tác", focus: "Mối quan hệ và đối tác", color: "from-blue-500 to-cyan-500" },
  3: { energy: "Sáng tạo", focus: "Biểu đạt và giao tiếp", color: "from-yellow-500 to-orange-500" },
  4: { energy: "Xây dựng", focus: "Công việc và kỷ luật", color: "from-green-500 to-teal-500" },
  5: { energy: "Tự do", focus: "Thay đổi và phiêu lưu", color: "from-purple-500 to-indigo-500" },
  6: { energy: "Chăm sóc", focus: "Gia đình và trách nhiệm", color: "from-pink-500 to-rose-500" },
  7: { energy: "Nội tâm", focus: "Học hỏi và phát triển", color: "from-indigo-500 to-purple-500" },
  8: { energy: "Thành công", focus: "Sự nghiệp và tài chính", color: "from-amber-500 to-yellow-500" },
  9: { energy: "Hoàn thành", focus: "Cống hiến và chia sẻ", color: "from-emerald-500 to-green-500" }
};
...
export default function NumerologyForecastPage() {
  ...
  const calculatePersonalNumber = (month: number, lifePath: number, year: number): number => {
    const sum = month + lifePath + year;
    return sum > 9 ? sum - 9 : sum;
  };

  const generateMonthlyForecast = (month: number, lifePath: number, year: number): MonthlyForecast => {
  ...
  return (
    <div className="min-h-screen ">
      {/* Header */}
      <div className="relative overflow-hidden bg-gradient-to-r from-purple-900/20 to-blue-900/20 py-20">
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-xl md:text-2xl text-gray-300 max-w-3xl mx-auto mb-4 leading-relaxed">
            <strong className="text-golden">Khám phá vận mệnh từng tháng</strong> trong năm {new Date().getFullYear()}
          </p>
          
          <p className="text-lg text-gray-400 max-w-2xl mx-auto leading-relaxed">
            Dự đoán chi tiết về cơ hội, thách thức và lời khuyên cho 12 tháng tới dựa trên thần số học
          </p>
        </div>
      </div>
              ...
              <div className="text-center mb-6">
                ...
                <div className="flex items-center justify-center gap-4 mb-4">
                  <div className="text-center">
                    <div className="text-golden font-semibold">Số đường đời</div>
                    <div className="text-2xl font-bold text-white">{forecast.lifePath}</div>
                  </div>
                  <div className="text-3xl">✨</div>
                  <div className="text-center">
                    <div className="text-golden font-semibold">Số năm cá nhân</div>
                    <div className="text-2xl font-bold text-white">{forecast.yearNumber}</div>
                  </div>
                </div>
              ...
              {forecast.months.map((month) => (
                ...
                >
                  <div className="text-center mb-4">
                    <div className={`w-16 h-16 bg-gradient-to-r ${month.color} rounded-full flex items-center justify-center text-2xl mb-3 mx-auto`}>
                      <span className="text-white font-bold">{month.personalNumber}</span>
                    </div>
                    <h3 className="text-xl font-bold text-white mb-2">{month.monthName}</h3>
                    <div className="text-golden font-semibold">{month.energy}</div>
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          {month.luckyDays.map((day) => (
                            <span key={day} className="bg-blue-500/20 text-blue-400 px-2 py-1 rounded text-sm">
                              {day}
                            </span>
                          ))}
                        </div>
